...
function App() {
  ...
  // Filters State
  const [filterUser, setFilterUser] = useState([]); // Array de user_name strings
  ...
  // Derive unique user names from trips
  const uniqueUserNames = useMemo(() => {
    return [...new Set(trips.map(t => t.user_name).filter(Boolean))].sort();
  }, [trips]);
  ...
  const filteredPoints = useMemo(() => {
    ...
    if (filterUser.length > 0) {
      const userTripIds = trips.filter(t => filterUser.includes(t.user_name)).map(t => t.id);
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter(t => filterUser.includes(t.user_name));
  }, [trips, filterUser]);
        ...
        <header className="filters-header glass">
          ...
          <div className="filter-group-container">
            <MultiSelectDropdown
              ...
              icon={Users}
              options={uniqueUserNames.map(name => ({ id: name, label: name }))}
              selected={filterUser}
              onChange={(id) => {
                if (id === 'all') setFilterUser([...uniqueUserNames]);
            ...
            <Marker key={point.id} position={[point.latitude, point.longitude]} icon={getMarkerIcon(point.type)}>
              <Popup className="custom-popup">
                <div className="popup-content">
                  <h4>{point.type}</h4>
                  <p className="popup-user">{trips.find(t => t.id === point.trip_id)?.user_name || 'Desconocido'}</p>
